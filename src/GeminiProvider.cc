...
namespace ai_chat_sdk
{
    ...
    {
        return "我是Google Gemini，由 Google 开发的人工智能助手";
    }
    ...
    std::string GeminiProvider::sendMessageStream(const std::vector<Message>& messages , const std::map<std::string , std::string>& requestParam , std::function<void(const std::string& , bool)> callback)
    {
        if(!isAvailable())
                {
                ...
                req.content_receiver = [&](const char* data , size_t data_length , size_t offset , size_t total_length)->bool
                {
                    ...
                    while((pos = buffer.find("\n\n")) != std::string::npos)
                    {
                        ...
                        if(chunk.compare(0 , 6 , "data: ") == 0)
                        {
                            ...
                            {
                                ERR("GeminiProvider sendMessageStream parse modelDataJson error: {}" , errors.c_str());
                            }
                            else
                            {
                                    ...
                                    && chunkDataJson["choices"][0].isMember("delta") && chunkDataJson["choices"][0]["delta"].isMember("content"))
                                {
                                    ...
                                    INFO("GeminiProvider sendMessageStream content: {}", content);
