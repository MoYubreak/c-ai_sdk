...
namespace ns_logger
{
    ...
    {
    public:
        static void initLogger(std::string& loggerName , std::string& logFile , spdlog::level::level_enum logLevel = spdlog::level::level_enum::info);
    ...
    #define CRI(format,...) Logger::getLogger()->critical(std::string("[{\:>10s}\:{\:<4d}]") + format , __FILE__ , __LINE__ , ##__VA_ARGS__)
}